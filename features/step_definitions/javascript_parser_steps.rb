Given /^I have loaded the sample JavaScript parser$/ do
  ...
  @lexer_class   = SmithereenSamples::SimplifiedJavaScriptLexer
  @parser_class  = SmithereenSamples::SimplifiedJavaScriptParser
  @grammar_class = SmithereenSamples::SimplifiedJavaScriptGrammar
end
...
When /^I ask for the parse tree for (\w+) "(.*)"$/ do |construct_type, js_source|
  ...
  (@names_to_define||[]).each{|name| parser.scope.define(Smithereen::LexerToken.new(:name, name))}
